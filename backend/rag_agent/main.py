...
def query_cohere(query: str, context_chunks: List[RetrievedChunk], temperature: float) -> (str, List[str], float):
    context_text = "\n\n".join([f"{c.content}" for c in context_chunks if c.content.strip()])
    prompt = f"CONTEXT:\n{context_text}\n\nQUESTION:\n{query}\nANSWER:"
    ...
    answer = response.generations[0].text.strip()
    sources = [c.url for c in context_chunks if c.url]
    confidence = min(1.0, 0.5 + 0.2 + 0.3 * (len(context_chunks)/len(context_chunks) if context_chunks else 0))
...
def query_endpoint(request: QueryRequest):
    try:
        chunks = retrieve_chunks(request.query, request.max_results)
        answer, sources, confidence = query_cohere(request.query, chunks, request.temperature)
        return QueryResponse(
            answer=answer,
            query=request.query,
